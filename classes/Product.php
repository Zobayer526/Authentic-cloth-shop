	...
	class Product{
		...
		public function productInsert($data,$files){
			...
			if ($productName==""||$cat_id==""||$clr==""||$product_size==""||$body==""||$price==""||$city=="" ||$qty==""||$status=="") {
	 		...
	 		$msg="Please enter only number value in price field";
		 	return $msg;
	 		}elseif (!is_numeric($qty)) {
	 		$msg="Please enter only number value in qty field";
		 	return $msg;
	 		}
			$colors=[];
			$sizes=[];
	...
	public function getAllAdminDelivery()
	{
		$adminid=Session::get("userId");
		$query= "SELECT id, delivery_date, cus_order.* FROM delivery_order,cus_order 
		WHERE delivery_order.order_id=cus_order.orderId AND status='2'";
    	$result=$this->db->select($query);
	...
	public function getAllOrderItems($id)
	{
		...
		public function getAllProductlist(){
			$query="SELECT * FROM product,category WHERE product.cat_id=category.catId order by id DESC";
		...
		public function productUpdate($data,$files,$id){
			...
			if ($productName==""||$cat_id==""||$product_size==""||$body==""||$price==""||$city=="" ||$qty==""||$status=="") {
				$msg="<span class='error'> field must not be empty</span>";
				return $msg;
	 		}elseif (!ctype_alpha($productName)) {
	 		$msg="Please enter only alphabetic value in name field";
	...
	$query ="INSERT INTO rating(product_id,user_id,name,email,rating,review)
		...
		public function getAllCity()
		{
			...
			$query="SELECT DISTINCT name FROM district  ORDER   BY id DESC LIMIT 15";
			$result=$this->db->select($query);
			return $result;
		}
	}
